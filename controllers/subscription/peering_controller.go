...
// +kubebuilder:rbac:groups=subscription.rediscloud.kubeform.com,resources=peerings,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=subscription.rediscloud.kubeform.com,resources=peerings/status,verbs=get;update;patch

func (r *PeeringReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("peering", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Peering")
...
func (r *PeeringReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &subscriptionv1alpha1.Peering{}); err != nil {
			klog.Error(err, "unable to set up auditor", subscriptionv1alpha1.Peering{}.APIVersion, subscriptionv1alpha1.Peering{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&subscriptionv1alpha1.Peering{}).
