...
// CloudAccountReconciler reconciles a CloudAccount object
...
// +kubebuilder:rbac:groups=rediscloud.rediscloud.kubeform.com,resources=cloudaccounts,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=rediscloud.rediscloud.kubeform.com,resources=cloudaccounts/status,verbs=get;update;patch

func (r *CloudAccountReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("cloudaccount", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch CloudAccount")
...
func (r *CloudAccountReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &rediscloudv1alpha1.CloudAccount{}); err != nil {
			klog.Error(err, "unable to set up auditor", rediscloudv1alpha1.CloudAccount{}.APIVersion, rediscloudv1alpha1.CloudAccount{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&rediscloudv1alpha1.CloudAccount{}).
...
func (r *CloudAccountReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for CloudAccount")
			return nil
		}
		...
		resourceList := &rediscloudv1alpha1.CloudAccountList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list CloudAccount")
			return nil
		}
