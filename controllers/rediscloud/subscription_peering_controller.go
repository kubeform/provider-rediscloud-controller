...
// SubscriptionPeeringReconciler reconciles a SubscriptionPeering object
type SubscriptionPeeringReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=rediscloud.rediscloud.kubeform.com,resources=subscriptionpeerings,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=rediscloud.rediscloud.kubeform.com,resources=subscriptionpeerings/status,verbs=get;update;patch

func (r *SubscriptionPeeringReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("subscriptionpeering", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SubscriptionPeering")
...
func (r *SubscriptionPeeringReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &rediscloudv1alpha1.SubscriptionPeering{}); err != nil {
			klog.Error(err, "unable to set up auditor", rediscloudv1alpha1.SubscriptionPeering{}.APIVersion, rediscloudv1alpha1.SubscriptionPeering{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&rediscloudv1alpha1.SubscriptionPeering{}).
