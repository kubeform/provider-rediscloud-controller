...
// +kubebuilder:rbac:groups=cloud.rediscloud.kubeform.com,resources=accounts,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cloud.rediscloud.kubeform.com,resources=accounts/status,verbs=get;update;patch
...
func (r *AccountReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cloudv1alpha1.Account{}); err != nil {
			klog.Error(err, "unable to set up auditor", cloudv1alpha1.Account{}.APIVersion, cloudv1alpha1.Account{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cloudv1alpha1.Account{}).
...
func (r *AccountReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(fmt.Errorf("expected a Secret but go a %T", o), "failed to get secret for Account")
			return nil
		}
		...
		resourceList := &cloudv1alpha1.AccountList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list Account")
			return nil
		}
